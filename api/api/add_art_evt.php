...
function add_art_evt($settings, $objets){
	$bdd = $objets['bdd'];
	...
	if($id_club!=-1 AND club_autorise($id_club, $objets['user_info'])){
		$bdd->query("INSERT INTO evt_articles VALUES (NULL, '".utf8_decode(addslashes($settings['nom']))."', ".utf8_decode(addslashes($settings['prix'])).", ".$id_club.")");
		
		$id_art = $id = $bdd->lastInsertId();
		
		$bdd->query("INSERT INTO evt_listearticles VALUES (".$settings['id_evt'].", ".$id_art.", ".$settings['qte'].")");
